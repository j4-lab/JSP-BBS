	...
	<div class="container">
		<div class="row">
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				...
				%>
					<td><a href="secretCheck.jsp?board_id=<%= list.get(i).getBoard_id()%>">비밀글 입니다.</a></td>
				<%
					}else{
				%>
			...
			<%
				if(pageNumber!=1){
			%>
				<a href="board.jsp?pageNumber=<%=pageNumber -1 %>&commentNumber=1" class = "btn btn-success btn-arraw-left">이전</a>
			<%
// 				} if(boardDAO.nextPage(pageNumber+1)){
				} if(Math.ceil(boardDAO.countData()/10)>=pageNumber){
			%>
				<a href="board.jsp?pageNumber=<%=pageNumber +1 %>&commentNumber=1" class = "btn btn-success btn-arraw-right">다음</a>
			<%
				}
			%>
